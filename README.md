...
* **п. 6.** Лучше указать какой из лимитов превышен. Отображать полоски исчерпания лимитов (индикаторы). 
...
Далее при пролистывании нужно сделать виртуализацию, т.е.загружать и показывать не весь список, а только отображаемый в данный момент на экране (загрузка делается с запасом на несколько экранов и происходит фоново) 
...
| **API Обновления токенов** [^1]| Микросервис (REST / gRPC) | Публичная точка входа для мобильных приложений. Принимает PUSH-токены от них и обновляет их в БД. |
